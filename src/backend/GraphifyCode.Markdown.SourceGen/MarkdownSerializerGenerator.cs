...
public class MarkdownSerializerGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateToMarkdownMethod(CodeBuilder code, string headerName, List<PropertyHandler> handlers, IPropertySymbol? headerProperty)
    {
        code.Nest("public string ToMarkdown()", method =>
        {
            ...
            PropertyHandler? previousHandler = null;
            foreach (var handler in handlers)
            {
                handler.GenerateSerializationCode(method, 1, ref firstProperty, previousHandler);
    ...
    private abstract class CollectionOfObjectsPropertyHandler(IPropertySymbol property, ITypeSymbol elementType) : PropertyHandler(property)
    {
        ...
        protected static string? GetSubHeaderText(IPropertySymbol property)
        {
            var attribute = property.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.Name is "MarkdownSubHeaderAttribute" or "MarkdownSubHeader");

            if (attribute?.ConstructorArguments.Length > 0)
            {
                var firstArg = attribute.ConstructorArguments[0];
                if (firstArg.Value is string subHeaderText)
        ...
        public override void GenerateSerializationCode(CodeBuilder code, int currentLevel, ref bool firstProperty, PropertyHandler? previousHandler)
        {
            ...
            {
                // Original behavior: add separator line before collection
                code.Line($$"""
                    sb.Append('\n');
                    """);
            }
        ...
        public override void GenerateDeserializationCode(CodeBuilder code, int currentLevel)
        {
            ...
            // Determine header levels based on whether SubHeader is present
            var itemHeaderLevel = subHeaderText != null ? currentLevel + 1 : currentLevel;
            var itemHeader = GetHeaderPrefix(itemHeaderLevel);
            ...
            // If SubHeader is specified, check for and skip it
            if (subHeaderText != null)
            {
                var subHeader = GetHeaderPrefix(currentLevel);
                code.Line($$"""
                    if (index < lines.Length && lines[index] == "{{subHeader}} {{subHeaderText}}")
                    {
                        index++;
                        {{LineProcessingHelpers.GenerateSkipEmptyLinesCode()}}
                    }
            ...
            code.Line($$"""
                while (index < lines.Length && lines[index].StartsWith("{{itemHeader}} "))
                {
                    var itemStart = index;
                    index++;
                    ...
                    while (index < lines.Length && !lines[index].StartsWith("{{itemHeader}} "))
                    {
                        ...
                        if (!string.IsNullOrEmpty(lines[index]) && lines[index].StartsWith("#"))
                        {
                            ...
                            for (int h = 0; h < {{itemHeaderLevel + 1}}; h++)
                            {
                            ...
                            if (isSameOrHigherLevel && ({{itemHeaderLevel + 1}} >= lines[index].Length || lines[index][{{itemHeaderLevel + 1}}] != '#'))
                                break;
                        }
                        index++;
                    }
            ...
            {
                code.Line($$"""
                        var headerValue = lines[itemStart].Substring({{itemHeaderLevel + 2}});
            ...
            code.Line($$"""
                    ...
                    for (int i = itemStart + 1; i < index; i++)
                    {
                        ...
                        if (line.StartsWith("{{itemHeader}}"))
                            line = line.Substring({{itemHeaderLevel}});
                        itemLines.Add(line);
                    }
